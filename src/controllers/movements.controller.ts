...
export const getMovements = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    const result = await movementsService.getMovements(req.user.id, pageNumber, search);
...
export const transfer = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    const data = await movementsService.transfer(req.user.id, req.user.name, email, amount, cardId);
    res.status(200).json({ success: true, message: 'Transfer successful', data });
  } catch (err) {
