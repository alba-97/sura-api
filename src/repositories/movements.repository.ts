...
export const findAndCountTransactions = (userId: number, page: number, search: string) => {
  const size = 5;
  const offset = (page - 1) * size;
  const where: Record<string, unknown> = { userId };
  if (search) where.title = { [Op.iLike]: `%${search}%` };
  return Transaction.findAndCountAll({ where, order: [['date', 'DESC']], limit: size, offset });
};
...
export const countContactsByUser = (userId: number) => UserContact.count({ where: { userId } });
...
export const destroyContact = (contact: { destroy: () => Promise<unknown> }) => contact.destroy();
